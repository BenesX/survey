<template>
    <div :class="getQuestionClass(element)">
        <!-- <template v-if="element.isPanel">
          <div v-for="row in element.elements" :key="row.id" :id="row.id"></div>
        </template> -->
...
export class SurveyElementVue extends Vue {
  ...
  get hasErrorsOnTop() {
    ...
    // return this.survey.questionErrorLocation === "top";
  }
  get hasErrorsOnBottom() {
    return (
      !this.element.isPanel && this.survey.questionErrorLocation === "bottom"
      // this.survey.questionErrorLocation === "bottom"
    );
  }
  mounted() {
